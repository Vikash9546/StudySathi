...
export default function BattleArenaScreen() {
  ...
  useEffect(() => {
    ...
    const setupSocket = async () => {
      try {
        ...
        socket.on('battle:score_update', (data: { scores: Record<string, number>; userId: string }) => {
        ...
        socket.on('battle:ended', (data: { scores: Record<string, number>; winnerId: string }) => {
  ...
  const handleNextQuestion = async (lastAns: string | null) => {
    setSelectedAns(null);
      ...
      <ScrollView contentContainerStyle={styles.scrollContainer}>
        ...
        <View style={styles.optionsList}>
          {activeQ.options.map((option, idx) => {
            ...
            let letterBoxStyle: any = styles.optionLetterBox;
